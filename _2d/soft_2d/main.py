...
def run(logger, num_envs=16, hidden_dim=256, batch_size=1024, iterations=1000, log_interval=10, runs=3, t_runs=10):
    ...
    logger.info(" ")
    logger.info("Initializing Soft Goals 2D Waypoint Navigation Environment")
    ...
    for i in range(runs):
        ...
        ep, rew, agent = tl.train_mp(logger, envs, t_env, agent, opt, batch_size, iterations, log_interval, t_runs, render=True, fname=path+wps+"-wps")
        if i == 0:

...
def run(num_envs=16, hidden_dim=256, batch_size=1024, iterations=1000, log_interval=10, runs=5):
    envs = [tl.make_term_2d() for i in range(num_envs)]
    envs = SubprocVecEnv(envs)
    t_env = tenv.TrajectoryEnvTerm2D()
    state_dim = t_env.observation_space.shape[0]
    action_dim = t_env.action_space.shape[0]
    path = os.getcwd()+"/term_2d/"
    for i in range(runs):
        agent = ag.Agent(state_dim, hidden_dim, action_dim, 2)
        opt = torch.optim.Adam(agent.parameters(), lr=1e-4)
        ep, rew, agent = tl.train_term_mp(envs, t_env, agent, opt, batch_size, iterations, log_interval, render=False, fname=path+"gaussian_"+str(2))
        if i == 0:
